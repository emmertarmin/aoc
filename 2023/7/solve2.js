...
rl
    .on('line', (line) => {
        ...
        if (!hand.find(ch => ch !== 'J')) {
            ...
            const charWithMaxNotJ = hyst.sort((a, b) => b.freq - a.freq).find(item => item.ch !== 'J').ch
            hyst = hyst.map(item => {
                return {
                    ...item,
                    chAlt: item.ch === 'J' ? charWithMaxNotJ : item.ch
                }
            })
            hyst.forEach((item, i) => {
                hyst[i].freq = hyst.reduce((acc, curr) => curr.chAlt === item.chAlt ? acc + 1 : acc, 0)
            })
        }
    ...
    .on('close', () => {
        ...
        function sortFnc(a, b) {
            if (a.type.rank === b.type.rank) {
                const chrs = ['A', 'K', 'Q', 'T', '9', '8', '7', '6', '5', '4', '3', '2', 'J']
